...
@inject
def insert_recipe_keywords(
    rid: UUID,
    keywords: List[str],
    keywordservice: KeywordRepository = Depends(
        Provide[Container.keywordservice]
    ),  # wiring marker is Container.Keywordservice
):
    if keywords != None:
        keywords_list = list(keywords)
        keywordservice.insert_keywords(rid, keywords_list)
        return JSONResponse(
            content={"message": "inserted recipe keywords"}, status_code=201
        )
    else:
