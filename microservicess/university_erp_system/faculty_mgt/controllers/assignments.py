...
from faculty_mgt.services.assignments import (
...
def create_assignment(assignment: AssignmentRequest):
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "create assignment problem encountered"},
            status_code=500,
        )
...
def create_workbin(stud_id: int, faculty_id: int):
    ...
    if result == True:
        return JSONResponse(
            content={"message": "workbin %s is created successful" % (bin_id)},
            status_code=201,
        )
    else:
        return JSONResponse(
