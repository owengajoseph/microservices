...
def signup(uname: str, passwd: str):
    if type(uname) is None and type(passwd) is None:
        return {"message": "invalid user"}
    elif valid_users.get(uname) == uname:
...
def login_with_token(username: str, password: str, id: UUID):
    if valid_users.get(username) is None:
        ...
        if user.id is id and checkpw(password.encode(), user.passphrase):
            return user
        else:
            return {"message": "invalid user"}
...
def post_discussion(username: str, post: Post, post_type: PostType):
    if valid_users.get(username) is None:
        return {"message": "user does not exist"}
    elif not (discussion_posts.get(id) is not None):
